...
static int decode_type1(rtcm_t *rtcm)
{
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "LOG_TAG", "Need to print : length=%d",rtcm->len);
    ...
    while (i+40<=rtcm->len*8) {
        ...
        if (rtcm->nav.dgps) {
            ...
            rtcm->nav.dgps[sat-1].t0=rtcm->time;
            rtcm->nav.dgps[sat-1].prc=prc*(fact?0.32:0.02);
            rtcm->nav.dgps[sat-1].rrc=rrc*(fact?0.032:0.002);
            rtcm->nav.dgps[sat-1].iod=iod;
            rtcm->nav.dgps[sat-1].udre=udre;

//            __android_log_print(ANDROID_LOG_DEBUG, "LOG_TAG", "Need to print : sat=%d",sat);
//            __android_log_print(ANDROID_LOG_DEBUG, "LOG_TAG", "Need to print : fact=%d",fact);
            __android_log_print(ANDROID_LOG_DEBUG, "LOG_TAG", "Need to print : PRC=%lf",rtcm->nav.dgps[sat-1].prc);
            __android_log_print(ANDROID_LOG_DEBUG, "LOG_TAG", "Need to print : RRC=%lf",rtcm->nav.dgps[sat-1].rrc);
            __android_log_print(ANDROID_LOG_DEBUG, "LOG_TAG", "Need to print : iod=%d",iod);
            __android_log_print(ANDROID_LOG_DEBUG, "LOG_TAG", "Need to print : udre=%d",udre);
        }
        else
        {
            __android_log_print(ANDROID_LOG_DEBUG, "LOG_TAG", "Need to print : NO dgps");
        }
    }
    return 7;
}
...
//        將type3中的position x y z 輸出
        rtcmfile = fopen("/sdcard/GPSData/rtcm.txt","a");
        fprintf(rtcmfile, "POS[0]=%lf\n", rtcm->sta.pos[0]);
        fprintf(rtcmfile, "POS[1]=%lf\n", rtcm->sta.pos[1]);
        fprintf(rtcmfile, "POS[2]=%lf\n", rtcm->sta.pos[2]);
        fclose(rtcmfile);
    }
    else {
...
extern int decode_rtcm2(rtcm_t *rtcm)
{
    ...
    adjhour(rtcm,zcnt);
    staid=getbitu(rtcm->buff,14,10);//IDENTIFICATION NUMBER OF THE BASE STATION
    seqno=getbitu(rtcm->buff,37, 3);//NUMBER OF SEQUENCES
    stah =getbitu(rtcm->buff,45, 3);//STATE OF WORKING

//        將type輸出
    rtcmfile = fopen("/sdcard/GPSData/rtcm.txt","a");
    fprintf(rtcmfile, "Type=%d\n", type);
    fclose(rtcmfile);

    __android_log_print(ANDROID_LOG_DEBUG, "LOG_TAG", "Need to print : type=%d",type);
    __android_log_print(ANDROID_LOG_DEBUG, "LOG_TAG", "Need to print : staid=%d",staid);
    __android_log_print(ANDROID_LOG_DEBUG, "LOG_TAG", "Need to print : seqno=%d",seqno);
    __android_log_print(ANDROID_LOG_DEBUG, "LOG_TAG", "Need to print : stah=%d",stah);
    ...
    if (ret>=0) {
        ...
        __android_log_print(ANDROID_LOG_DEBUG, "LOG_TAG", "Need to print : nmsg=%d",rtcm->nmsg2[type]++);
    }
    return ret;
}
